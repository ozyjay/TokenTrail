...
TRACE_EXPLANATION = "Candidate bars show this token's top alternatives from the local model."
...
def load_hf_libraries() -> tuple[Any, Any, Any]:
    try:
        ...
        raise ProbeError(
            "Missing required dependency 'torch'. Install project dependencies with: poetry install"
        ) from error
    ...
    try:
        ...
        raise ProbeError(
            "Missing required dependency 'transformers'. Install project dependencies with: poetry install"
        ) from error
