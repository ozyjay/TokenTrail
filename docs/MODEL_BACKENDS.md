...
Token Trail discovers the configured public aliases `qwen-0-5b`, `qwen-1-5b` and `qwen-3b` through ModelDeck `GET /native/v1/capabilities`, preserving configuration order and the advertised `ready` state. It requests native traces through `POST /native/v1/autoregressive/traces`. It does not know deployment IDs or worker ports, and does not own lifecycle, warm-up, model downloads or GPU memory.
